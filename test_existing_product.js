...
// Test the new purchase fields display with an existing product
async function testExistingProduct() {
  try {
    ...
    if (purchasesData && purchasesData.data && purchasesData.data.values) {
      ...
      // Find our test purchase (should be near the end)
      const testPurchases = rows.filter(row => row[4] === 'COKE 600MLS 12S/W NP');
      if (testPurchases.length > 0) {
        // Get the most recent one
        const testPurchase = testPurchases[testPurchases.length - 1];
